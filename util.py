...
def pred_train_ML(model,train_data_x_in,train_dataset,total_train_in, total_train_out, num_feats):
    ...
    train_mape = numpy.sqrt(numpy.sum((train_dataset[:,num_feats] - pred_train)**2)) / numpy.sqrt(numpy.sum(train_dataset[:,num_feats]**2))
    
    

    return train_mae, train_r2, train_mape
...
def pred_test_ML(model,num_feats, test_data_x_in,test_data_y, total_test_in, total_test_out):    
    ...
    test_mape = numpy.sqrt(numpy.sum((test_data_y - pred_test)**2)) / numpy.sqrt(numpy.sum(test_data_y**2))
    
    
    
    return test_mae, test_r2, test_mape
    
def draw_pickup(thermo, ZT,doping):
    ...
    for i in range(30):
        ...
        id_pick_1.append(id[testttt.index.values[i]])
        id_pick_2.append(testttt.index.values[i])


    id_pick=list(id_pick_1)

    labellist=list(id_pick_2)

    aaa=[]
    for ttt in labellist:
        if id[ttt] in aaa:
            continue
        else:
            print(id[ttt],"Sn",thermo["Sn_ratio"][ttt],doping,thermo["dopant1_ratio"][ttt],\
              str(ELEMENTS[thermo["dopant2_num"][ttt]].symbol), thermo["dopant2_ratio"][ttt], "Se", thermo["Se_ratio"][ttt])
        aaa.append(id[ttt])
    ...
    for i in range(len(temp)):
        ...
        if id[i] == id_pick[n]:
            ...
            if i == len(temp)-1:
                ax.plot(temp_pick,zt_pick,marker="o", label=id[i])
...
def dopantremove(drop_num, chosen):
    measureset=['electrical_conductivity','seebeck_coeff','thermal_conductivity']

    #remove all the data containing dopant with # of drop_num
    for measure in measureset:


        #training info
        file = './inputdata/'+str(measure)+'.csv'
        ...
        pandas.DataFrame(remove).to_csv('./outputdata_V_test/'+str(measure)+'_dopantremove.csv')
        ...
        pandas.DataFrame(new_feature).to_csv('./outputdata_V_test/training_features_'+str(measure)+'_dopantremove.csv')


    #ZT test info
    zt_info  = pandas.read_csv('./inputdata/zt_dataset.csv', index_col=0)
    survive = zt_info[(zt_info['dopant2_num'] == drop_num) | (zt_info['dopant1_num'] == drop_num)]
    survive.reset_index(inplace=True, drop=True)
    pandas.DataFrame(survive).to_csv('./outputdata_V_test/zt_dataset_dopantremove.csv')

    zt_feature = pandas.read_csv('./inputdata_feature/zt_test_features.csv', index_col=0)
    survive = zt_feature[zt_feature['dopant_num'] == drop_num]
    survive.reset_index(inplace=True, drop=True)
    pandas.DataFrame(survive).to_csv('./outputdata_V_test/zt_test_features_dopantremove.csv')    


    #remove the data containing dopant with # of drop_num only without the chosen data
    for measure in measureset:
        ...
        pandas.DataFrame(remove).to_csv('./outputdata_V_test/'+str(measure)+'_dopantremove_exist.csv')
        ...
        new_feature.reset_index(inplace=True, drop=True)
        pandas.DataFrame(new_feature).to_csv('./outputdata_V_test/training_features_'+str(measure)+'_dopantremove_exist.csv')
    ...
    survive.reset_index(inplace=True, drop=True)
    pandas.DataFrame(survive).to_csv('./outputdata_V_test/zt_dataset_dopantremove_exist.csv')

    zt_feature = pandas.read_csv('./inputdata_feature/zt_test_features.csv', index_col=0)
    survive = zt_feature[zt_feature['dopant_num'] == drop_num]
    survive = survive[(survive['id'] != chosen)]
    survive.reset_index(inplace=True, drop=True)
    pandas.DataFrame(survive).to_csv('./outputdata_V_test/zt_test_features_dopantremove_exist.csv')     
